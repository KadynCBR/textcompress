...
// Taken from: https://stackoverflow.com/questions/28050669/can-i-report-progress-for-openmp-tasks
#pragma once
...
class ProgressBar {
 private:
  ...
  uint32_t work_done;
  uint16_t old_percent;  ///< Old percentage value (aka: should we update the progress bar) TODO: Maybe that we do not
                         ///< need this
 ...
 public:
  ...
  void update(uint32_t work_done0) {
    ...
    //   [================================================  ] (96% - 1.0s - 4 threads)
    std::cerr << "\r\033[2K[" << std::string(percent / 2, '=') << std::string(50 - percent / 2, ' ') << "] (" << percent
              << "% - " << std::fixed << std::setprecision(1) << timer.lap() / percent * (100 - percent) << "s - "
